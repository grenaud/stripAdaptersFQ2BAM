...
int main (int argc, char *argv[]) {
	...
	cout<<"\t\t-q  [qual]"<<"\t\t"<<"If input is fasta, use this qual as the quality (Default : "+stringify(qualForFasta)+")"<<endl;
        ...
        cout<<"\t\t\t-fl [linker]"     <<"\t\t"<<"Use this forward linker"<<endl;
        cout<<"\t\t\t-rl [linker]"     <<"\t\t"<<"Use this as reverse linker"<<endl;
        cout<<"\t\t\t-e [editDistance]"<<"\t\t"<<"Max edit distance for linkers  (Default : "+stringify(linkerEdit)+")"<<endl;
	...
	if(strcmp(argv[i],"-fl") == 0  ){
	    linkerF=string(argv[i+1]);
            i++;
            continue;
        }

	if(strcmp(argv[i],"-rl") == 0  ){
	    linkerR=string(argv[i+1]);
            i++;
            continue;
        }

	if(strcmp(argv[i],"-e") == 0  ){
            linkerEdit=destringify<int>(argv[i+1]);
            i++;
            continue;
        }
